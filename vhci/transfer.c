...
int bce_vhci_transfer_queue_do_pause(struct bce_vhci_transfer_queue *q)
{
    ...
    if (q->sq_out) {
        ...
        pr_info("bce-vhci: [%02x] pause: waiting for pending OUT requests (%d pending)\n",
                q->endp_addr, pending);
        wait_ret = wait_event_timeout(q->sq_out_wait_queue,
                                      atomic_read(&q->sq_out_pending) == 0,
                                      msecs_to_jiffies(2000));
        if (!wait_ret) {
            pr_warn("bce-vhci: [%02x] pause: pending output wait timed out after 2000ms (%d pending), forcing pause/flush\n",
                    q->endp_addr, atomic_read(&q->sq_out_pending));
        } else {
            pr_info("bce-vhci: [%02x] pause: pending OUT requests drained\n", q->endp_addr);
        }
    }
    bce_vhci_transfer_queue_remove_pending(q);
