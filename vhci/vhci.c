...
static int bce_vhci_hub_control(struct usb_hcd *hcd, u16 typeReq, u16 wValue, u16 wIndex, char *buf, u16 wLength)
{
    ...
    /*
     * During system PM resume we intentionally skip host-side PORT_RESUME.
     * Outside that window, hub control must keep normal runtime behavior.
     */
    if (vhci->system_resume_active &&
        typeReq == ClearPortFeature &&
        wValue == USB_PORT_FEAT_SUSPEND) {
        pr_info_ratelimited("bce-vhci: skipping host port-resume request for port %u during system resume\n",
                            wIndex);
        return 0;
    }
    ...
    if (typeReq == GetHubDescriptor && wLength >= sizeof(struct usb_hub_descriptor)) {
        ...
        pr_warn_ratelimited("bce-vhci: ignoring unsupported ClearPortFeature wValue=%u wIndex=%u\n",
                            wValue, wIndex);
        return 0;
    }
...
static int bce_vhci_bus_suspend(struct usb_hcd *hcd)
{
    ...
    pr_info("bce_vhci: suspend ports skipped\n");
...
static int bce_vhci_bus_resume(struct usb_hcd *hcd)
{
    int status;
    struct bce_vhci *vhci = bce_vhci_from_hcd(hcd);
    vhci->system_resume_active = true;
    pr_info("bce_vhci: resume started\n");
    bce_vhci_command_queue_clear_pending(&vhci->cq, "resume start");

    /* Resume event queues first so command/system replies can be received. */
    ...
    /*
     * macOS-like cold re-init sequence:
     * 1) controller_enable (fresh T2-side VHCI state)
     * 2) controller_start
     */
    ...
    /*
     * macOS-like warm resume:
     * do not force host-side port power/re-enumeration here.
     * T2 firmware emits port status change events as devices become ready.
     */
    status = 0;

out:
    bce_vhci_command_queue_clear_pending(&vhci->cq, status ? "resume failed" : "resume complete");
    pr_info("bce_vhci: resume %s\n", status ? "FAILED" : "done");
    if (status) {
        /* Don't leave resume window open on failure. */
        vhci->system_resume_active = false;
    }
    return status;
}
